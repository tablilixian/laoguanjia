...
class ItemDetailPage extends ConsumerWidget {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildSection(
                          ...
                          child: typesAsync.when(
                            ...
                            error: (_, __) => Text(_getTypeLabel(item.itemType)),
                            data: (types) {
                              final type = types.where((t) => t.typeKey == item.itemType).firstOrNull;
                              return Row(
                                children: [
                      ...
                      Expanded(
                        child: _buildSection(
                          title: '位置',
                          child: item.locationName != null
                              ? Row(
                                  children: [
                                    Text(item.locationIcon ?? '📍', style: const TextStyle(fontSize: 18)),
                                    const SizedBox(width: 8),
                  ...
                  // 购买日期和价格
                  if (item.purchaseDate != null || item.purchasePrice != null)
                    Row(
                      children: [
                        ...
                        if (item.purchasePrice != null)
                          Expanded(
                            child: _buildSection(
                              title: '购买价格',
                              child: Text('¥ ${item.purchasePrice!.toStringAsFixed(2)}'),
                            ),
                          ),
                      ],
                    ),
                        ...
                        children: item.tags.map((tag) {
                          return Chip(
                            ...
                            backgroundColor: tag.color != null
                                ? Color(int.parse(tag.color!.replaceFirst('#', '0xFF'))).withOpacity(0.2)
                                : null,
                          );
                        }).toList(),
                      ),
  ...
  Widget _buildWarrantyStatus(DateTime warrantyExpiry) {
    ...
    if (daysLeft < 0) {
      ...
      return Chip(
        label: Text('还剩 $daysLeft 天', style: const TextStyle(color: Colors.orange)),
        backgroundColor: Colors.orange.shade50,
      );
    } else {
      final years = (daysLeft / 365).floor();
      return Chip(
        label: Text('还有 ${years > 0 ? "$years 年" : "$daysLeft 天"}', style: const TextStyle(color: Colors.green)),
  ...
  void _showDeleteDialog(BuildContext context, WidgetRef ref, HouseholdItem item) {
    showDialog(
      context: context,
