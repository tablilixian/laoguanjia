...
class TagsState {
  ...
  int get totalCount => tags.length;
...
class TagsNotifier extends StateNotifier<TagsState> {
  ...
  Future<void> _loadTags() async {
    ...
    try {
      ...
      print('🔵 [TagsNotifier] 加载标签: ${tags.length} 个');
      state = state.copyWith(tags: tags, isLoading: false);
    } catch (e) {
      print('🔴 [TagsNotifier] 加载失败: $e');
  ...
  Future<void> deleteTag(String tagId) async {
    ...
    try {
      ...
      state = state.copyWith(
        isLoading: false,
        errorMessage: '删除标签失败: ${e.toString()}',
      );
    }
  }
...
final offlineTagsProvider = StateNotifierProvider<TagsNotifier, TagsState>((ref) {
