...
|----------|-----------|------|-------|------------|
| clothing | 衣物 | 👕 | #E91E63 | 1 |
| appliance | 家电 | 🔌 | #2196F3 | 2 |
| furniture | 家具 | 🛋️ | #795548 | 3 |
| daily | 日用品 | 🧴 | #4CAF50 | 4 |
| tableware | 餐具 | 🍽️ | #FF9800 | 5 |
| food | 食品调料 | 🥫 | #8BC34A | 6 |
| bedding | 床上用品 | 🛏️ | #9C27B0 | 7 |
| electronics | 电子数码 | 📱 | #3F51B5 | 8 |
| book | 书籍 | 📚 | #673AB7 | 9 |
| decoration | 装饰品 | 🖼️ | #00BCD4 | 10 |
| tool | 工具 | 🔧 | #607D8B | 11 |
| medicine | 药品 | 💊 | #F44336 | 12 |
| sports | 运动器材 | ⚽ | #00BCD4 | 13 |
| toy | 玩具 | 🎮 | #FF5722 | 14 |
| jewelry | 珠宝首饰 | 💍 | #FFD700 | 15 |
| pet | 宠物用品 | 🐕 | #8D6E63 | 16 |
| garden | 园艺绿植 | 🌱 | #4CAF50 | 17 |
| automotive | 车载物品 | 🚗 | #455A64 | 18 |
| stationery | 文具办公 | 📎 | #78909C | 19 |
| consumables | 消耗品 | 🧻 | #90A4AE | 20 |
| other | 其他 | 📦 | #9E9E9E | 99 |

> **设计说明**：采用全局预设模式，21种类型对所有用户可见，新用户无需初始化即可直接使用。

**扩展性设计**：
- 家庭可添加自定义类型（如"收藏品"、"乐器"、"摄影器材"）
- 家庭可停用不需要的类型（is_active=false）
- 停用不影响已有物品数据
...
```
┌─────────────────────────────────────────────────────────┐
│  全局预设类型 (household_id = NULL)                     │
│  - 21种常用类型，所有用户共享                           │
│  - 不可编辑/删除                                        │
│  - 新用户直接可用，无需初始化                           │
├─────────────────────────────────────────────────────────┤
│  自定义类型 (household_id = 家庭ID)                    │
│  - 家庭可以添加自己的特殊类型                          │
│  - 可编辑/停用/删除                                    │
└─────────────────────────────────────────────────────────┘
```
...
**关联机制**：
- `household_items.item_type` 存储的是 `type_key` 字符串（如 `'appliance'`）
- 通过 `type_key` 去 `item_type_configs` 表查找对应的 label、icon、color
- 这是"软关联"，不需要外键约束，更加灵活
...
CREATE TABLE item_tags (
  ...
  -- 适用物品类型（新增 v1.2）
  applicable_types TEXT[] DEFAULT '{}',          -- 适用的物品类型，为空表示适用于所有类型
...
**标签分组 category 可选值（共11种）**：

| category | 名称 | 说明 | 典型标签 |
|----------|------|------|----------|
| season | 季节 | 物品对应的季节 | 春装、夏装、秋装、冬装 |
| color | 颜色 | 物品外观颜色 | 深色、浅色、红色、蓝色 |
| status | 状态 | 使用状态 | 全新、正常使用、磨损、需要维修 |
| warranty | 保修 | 保修相关 | 在保修期内、保修过期、快到期 |
| ownership | 归属 | 谁的东西/是否共享 | 我的、家人专属、共用、借出 |
| storage | 存放方式 | 如何存放 | 收纳箱、抽屉、柜子、悬挂 |
| frequency | 使用频率 | 多久用一次 | 常用、偶尔用、很少用 |
| value | 价值 | 价值分类 | 高价值、纪念品、便宜货 |
| source | 来源 | 如何获得 | 购买、礼物、二手、自己制作 |
| disposition | 处理意向 | 将来想怎么处理 | 保留、送人、捐赠、变卖、丢弃 |
| other | 其他 | 不属于以上分类 | 自定义标签 |

**适用物品类型 applicable_types**：
- 为空数组 `{}` = 适用于所有物品类型
- 如 `{'appliance', 'furniture'}` = 仅适用于家电和家具类型
- 创建物品时，标签会根据当前选择的物品类型自动排序和显示/隐藏

**预设标签（初始化时插入）**：

| 名称 | 分组 | 适用类型 | 颜色 |
|------|------|----------|------|
| 春装 | season | clothing | #4CAF50 |
| 夏装 | season | clothing | #FF9800 |
| 秋装 | season | clothing | #795548 |
| 冬装 | season | clothing | #2196F3 |
| 深色 | color | 全部 | #424242 |
| 浅色 | color | 全部 | #9E9E9E |
| 彩色 | color | 全部 | #E91E63 |
| 全新 | status | 全部 | #4CAF50 |
| 正常使用 | status | 全部 | #2196F3 |
| 有些磨损 | status | 全部 | #FF9800 |
| 需要维修 | status | 全部 | #F44336 |
| 在保修期内 | warranty | appliance | #4CAF50 |
| 保修即将到期 | warranty | appliance | #FF9800 |
| 保修已过期 | warranty | appliance | #9E9E9E |
| 常用 | frequency | 全部 | #4CAF50 |
| 偶尔用 | frequency | 全部 | #2196F3 |
| 很少用 | frequency | 全部 | #9E9E9E |
| 高价值 | value | 全部 | #F44336 |
| 纪念品 | value | 全部 | #9C27B0 |
| 送人 | disposition | 全部 | #E91E63 |
| 捐赠 | disposition | 全部 | #4CAF50 |
| 待处理 | disposition | 全部 | #FF9800 |
...
| v1.2 | 2026-03-14 | 增强：标签系统升级 - 11种分类大类、支持类型优先级（applicable_types）、自定义分类名称图标 |
| v1.3 | 2026-03-16 | 优化：物品类型改为全局预设模式（21种类型），新用户直接可用；类型管理页面支持查看预设和自定义 |
| v1.4 | 2026-03-18 | 新增：AI物品助手页面 - 支持AI创建物品、AI查询物品、AI统计功能 |
...
#### 预设类型
- 21种预设类型对所有用户可见，无需初始化
- 预设类型为只读（显示锁图标），不可编辑/删除
- 预设类型包括：衣物、家电、家具、日用品、餐具、食品调料、床上用品、电子数码、书籍、装饰品、工具、药品、运动器材、玩具、珠宝首饰、宠物用品、园艺绿植、车载物品、文具办公、消耗品、其他
...
| 标签类型 | 显示效果 |
|---------|---------|
| 适用当前类型的标签 | 🟢 顶部显示，带绿色"适用"标签，可正常选择 |
| 不适用当前类型的标签 | ⚪ 灰色显示，不可选择（但已选中的仍可取消） |
...
| 季节 | 春装、夏装、秋装、冬装 | 衣物 |
| 颜色 | 深色、浅色、红色、蓝色 | 所有 |
| 状态 | 全新、正常使用、有些磨损、需要维修 | 所有 |
| 保修 | 在保修期内、保修已过期、快到期 | 家电 |
| 归属 | 我的、家人专属、共用、借出 | 所有 |
| 存放方式 | 收纳箱、抽屉、柜子、悬挂 | 所有 |
| 使用频率 | 常用、偶尔用、很少用 | 所有 |
| 价值 | 高价值、纪念品、便宜货 | 所有 |
| 来源 | 购买、礼物、二手、自己制作 | 所有 |
| 处理意向 | 保留、送人、捐赠、变卖、丢弃 | 所有 |
...
```sql
-- 添加 applicable_types 列到 item_tags 表
ALTER TABLE item_tags 
ADD COLUMN IF NOT EXISTS applicable_types TEXT[] DEFAULT '{}';
```

### 9.7 AI 物品助手

AI物品助手是一个专门为物品管理设计的AI对话入口，提供自然语言处理功能。
...
- **意图识别**：通过关键词检测判断用户意图（create/query/stats）
- **AI服务**：`lib/data/ai/ai_item_service.dart`
- **对话UI**：`lib/features/items/pages/item_ai_assistant_page.dart`
- **路由**：`/home/items/ai`
